...
<Callout type="caution">
  ...
  もしくは、 [`trash-cli`](https://github.com/andreafrancia/trash-cli) の
...
コマンド文字列内では `unzip` コマンドを使う（[unzip コマンドで ZIP ファイルからファイルを取り出す](/note/command-unzip) も参照）。その引数として `xargs` に渡されたものを使用する。
