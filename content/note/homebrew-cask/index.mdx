...
App Store のアプリケーションを Homebrew 経由で管理することも可能。 → [mas-cli を使って Homebrew 経由で App Store のアプリケーションを管理する](/note/homebrew-mas-cli)

また、Homebrew 経由でインストールしたものは、Homebrew Bundle を使って Brewfile にリストアップしておくことも可能なので、パソコンを新しくするときなどに一括でインストールできるのも便利。 → [Homebrew Bundle を使って Brewfile でパッケージを管理する](/note/homebrew-bundle)
