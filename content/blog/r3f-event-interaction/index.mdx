...
function DiscoLights() {
  ...
  useFrame(({ clock }) => {
    lightsRef.current.forEach((light, index) => {
      ...
      if (index % 2 === 0) {
        light.position.x = Math.sin(clock.elapsedTime * speed + offset) * radius
        light.position.y = Math.cos(clock.elapsedTime * speed + offset) * radius
      } else {
        light.position.y = Math.sin(clock.elapsedTime * speed + offset) * radius
        light.position.z = Math.cos(clock.elapsedTime * speed + offset) * radius
      }
    })
  })
